...
let sunDir = [1, 0, 0.4], sunDirFlip=[0,0,0];
...
function drawShadedMap() {
  ...
  function f(xNorm) {
    ...
    ctx.fillText('\u03C6  = 0', canvas.width/2 - 35, y-10);
...
// North Pole
    ...
    northPole.position.copy(new THREE.Vector3(0,1,0));
    scene.add(northPole);
    ...
    southPole.position.copy(new THREE.Vector3(0,-1,0));
    scene.add(southPole);
...
// Labels  
  ...
  const nLabel = makeTextSprite("N", new THREE.Vector3(0, 1.1, 0.1),"orange","70px Arial");
  const sLabel = makeTextSprite("S", new THREE.Vector3(0, -1.1, 0.1),"lightblue","70px Arial");
  scene.add(xLabel, yLabel, zLabel, nLabel,sLabel);
  ...
  const arc = createArcAroundAxis(1.01, 0, Math.PI , 128, axis, new THREE.Vector3(0,1,0), "lightgreen");
  globe.userData.terminator = arc;
  scene.add(arc);

  const arc2 = createArcAroundAxis(1.01, 0, Math.PI, 128, axis, new THREE.Vector3(0,-1,0), "red");
  globe.userData.terminator2 = arc2;
  scene.add(arc2);

  const equator = createArcAroundAxis(1.01, 0, Math.PI * 2, 128, up, new THREE.Vector3(1,0,0), 'white');
  globe.userData.equator = equator;
  scene.add(equator);
...
function updateGlobeLight() {
  ...
  const newArc = createArcAroundAxis(1.01, 0, Math.PI, 128, axis, new THREE.Vector3(0,1,0), "lightgreen");
  scene.remove(globe.userData.terminator);
  globe.userData.terminator = newArc;
  scene.add(newArc);

  const newArc2 = createArcAroundAxis(1.01, 0, Math.PI, 128, axis, new THREE.Vector3(0,-1,0), "red");
  scene.remove(globe.userData.terminator2);
  globe.userData.terminator2 = newArc2;
  scene.add(newArc2);
}
